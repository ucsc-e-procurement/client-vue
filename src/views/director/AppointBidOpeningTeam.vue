      ...
      <v-form>
        ...
        <v-row>
          <v-btn
            color="success"
            class="mr-4 ml-4"
            @click="this.verifyBidOpeningTeam"
          >
            Assign
          </v-btn>
        </v-row>
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn
                color="blue darken-1"
                text
                @click="this.assignBidOpeningTeam"
                >Confirm</v-btn
              >
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    getEmployeesNotInTecTeam() {
      this.$http
        .get(
          `/api/director/get_employees_not_in_tec_team?tecTeamId=${this.tecTeamId}`
        )
        .then(response => {
    ...
    getMaxBidOpeningTeamId() {
      this.$http
        ...
        .then(response => {
          console.log(response);
          this.maxId =
            response.data[0].maxBidTeamId == null
              ? 0
              : response.data[0].maxBidTeamId;
        })
        .catch(err => {
          console.log(err);
        });
