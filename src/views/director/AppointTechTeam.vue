              ...
              <template v-slot:item="data">
                ...
                <v-tooltip bottom color="#E0E0E0">
                  ...
                  <span v-if="data.item.assigned">
                    <v-banner
                      two-line
                      v-for="item in JSON.parse(data.item.assigned)"
                      v-bind:key="item.procurementId"
                    >
      ...
      <v-row justify="center">
          ...
          <v-card>
              ...
              <v-container>
                ...
                <v-col cols="12" sm="12">
                  <v-list>
                      ...
                      <v-text-field
                        :value="
                          Object.assign(
                            {},
                            ...people.filter(i =>
                              this.chairman.includes(i.employee_id)
                            )
                          ).name
                        "
                        outlined
                    ...
                    <v-list-item v-for="item in team" :key="item">
                      <v-text-field
                        :value="
                          people.filter(i => i.employee_id == item)[0].name
                        "
                        outlined
                        readonly
                        label="Member"
            ...
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="dialog = false"
                >Close</v-btn
              >
              <v-btn color="blue darken-1" text @click="this.assignTecTeam"
                >Confirm</v-btn
              >
            </v-card-actions>
          </v-card>
        ...
        <v-dialog v-model="error" max-width="290">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="error = false">
                Dismiss
              </v-btn>
            </v-card-actions>
          </v-card>
    ...
    getMaxTecTeamId() {
      this.$http
        ...
        .then(response => {
          console.log(response);
          this.maxId =
            response.data[0].maxTecId == null ? 0 : response.data[0].maxTecId;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    filterList() {
      if (!this.chairman || this.selectedMemberCount == "") {
        ...
        return this.people.filter(
          item => !this.chairman.includes(item.employee_id)
        );
      } else {
