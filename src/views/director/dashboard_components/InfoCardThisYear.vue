          ...
          <div class="d-flex flex-no-wrap justify-space-between">
            <div>
              ...
              >
                <div style="font-size: 50px;">
                  {{
                    Math.round(this.completedProcurementCount * 100 * 100) /
                      100
                  }}%
                </div>
              </v-card-title>
    ...
    getProcurements() {
      this.$http
        ...
        .then(response => {
          ...
          var totalProcurements =
            response.data.filter(item => item.status == "on-going").length +
            response.data.filter(item => item.status == "completed").length +
            response.data.filter(item => item.status == "terminated").length;
          this.ongoingProcurementCount =
            response.data.filter(item => item.status == "on-going").length /
            totalProcurements;
          this.completedProcurementCount =
            response.data.filter(item => item.status == "completed").length /
            totalProcurements;
          this.terminatedProcurementCount =
            response.data.filter(item => item.status == "terminated").length /
            totalProcurements;
          this.requisitionCount = response.data.filter(
            item =>
