    ...
    <v-form>
      <v-card class="overflow-hidden" color="white">
        ...
        <v-dialog v-model="dialog2" width="500">
          <v-card>
            <v-card-title
              ><h4 class="ml-6">Reasons for denial</h4></v-card-title
            >
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$http
        .get(
          `/api/deputy_bursar/product_requisition/details?requisitionId=${this.$route.query.id}`
        )
        .then(response => {
          console.log(this.id);
    ...
    approveRequisition() {
      this.$http
        .post(
          `/api/deputy_bursar/product_requisition/approve?requisitionId=${this.$route.query.id}&selectedFundType=${this.selectedFundType}`
        )
        .then(response => {
          console.log(response);
    ...
    denyRequisition() {
      this.$http
        .post(
          `/api/deputy_bursar/product_requisition/deny?requisitionId=${this.$route.query.id}&remarks=${this.remarks}`
        )
        .then(response => {
