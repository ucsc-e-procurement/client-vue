  ...
  <v-container>
        <v-card flat>
          <template v-for="(item,key) in completedOrders">
            <v-card :key="item.procurement_id"
              class="mx-auto"
              max-width="800"
              flat
            >
            ...
            <v-divider :key="key" v-if="key != allCompletedOrders.length - 1"></v-divider>
          </template>
        </v-card>
        <template>
...
export default {
  ...
  methods: {
    ...
    getCompletedOrders(supplier_id) {
      this.$http.get('/api/supplier/get_completed_orders', {
        params: {
          id: supplier_id
        }
      })
  ...
  computed: {
    completedOrders () {
      return this.allCompletedOrders.slice((this.page - 1)* this.perPage, this.page* this.perPage)
    }
  }
};
</script>
