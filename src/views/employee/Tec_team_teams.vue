<template>
  <v-container fluid class="px-0 py-0">
                            ...
                            <v-card flat>
                              ...
                              >
                                <template v-slot:item.controls="props">
                                  ...
                                  <v-btn class="mx-2" small color="primary" @click="openTeam(props.item)">
                                    TEAM
                                  </v-btn>
                                </template>
                              </v-data-table>
    ...
    <v-dialog  v-if="dialog" :team="tec_team" :procurement ="procurement" v-model="dialog" width="650px">
        <v-card>
            <v-card-title>
            <span class="headline">TEC Team ID : {{procurement.tec_team_id}}</span>
            </v-card-title>
            <v-card-text>
...
export default {
  ...
  data: () => ({
    ...
    pastTeams: [],
    headers: [
        { text: 'Team ID', align: 'start', filterable: true, value: 'tec_team_id'},
  ...
  methods: {
    ...
    fetchCurrentTeams(employee_id) {
      this.$http.get('/api/tec_team/get_current_tec_teams', {
        params: {
          id: employee_id
        }
      })
    ...
    fetchPastTeams(employee_id) {
      this.$http.get('/api/tec_team/get_past_tec_teams', {
        params: {
          id: employee_id
        }
      })
